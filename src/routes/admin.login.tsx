      ...
      <form onSubmit={submit} className="w-full max-w-sm bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
        ...
        {mode === "signup" && (
          <>
            ...
            <input type="text" value={inviteCode} onChange={e => setInviteCode(e.target.value)} className="mt-1 w-full border border-slate-300 rounded px-3 py-2 text-sm outline-none focus:border-terracotta font-mono" />
            <p className="mt-1 text-[11px] text-slate-500">Leave blank if your email is on the admin allowlist; otherwise enter the invite code.</p>
          </>
        )}
